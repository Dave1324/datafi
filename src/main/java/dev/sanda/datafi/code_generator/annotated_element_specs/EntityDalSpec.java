...
public class EntityDalSpec extends AnnotatedElementSpec<TypeElement> {
    ...
    public Map<String, TypeName> getEntityFieldTypes(){
        return fieldDalSpecs.stream().collect(Collectors.toMap(
    ...
    private void setFieldSpecs(TypeElement entityApiSpec){
        ...
        val gettersByFieldNames = apiSpecGetters.stream().collect(Collectors.toMap(
                getter -> getter.getSimpleName().toString().replaceFirst("get", ""),
                Function.identity()
        ));
        entityFields.forEach(field -> {
            val getter = gettersByFieldNames.get(toPascalCase(field.getSimpleName().toString()));
            if(getter != null)
                this.fieldDalSpecs.add(new FieldDalSpec(field, getter));
            else
                this.fieldDalSpecs.add(new FieldDalSpec(field));
        });
    }
