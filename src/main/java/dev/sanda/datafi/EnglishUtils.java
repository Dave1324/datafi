...
/**
 * code from 'http://www.yorku.ca/jmason/EnglishNoun.java'
 * */
...
public class EnglishUtils {
  ...
  public static boolean isPlural(String word) {
    ...
    if (
      word.length() >= 2 && word.charAt(word.length() - 2) == 's'
    ) return false;
    return true;
  }
  ...
  public static String pluralOf(String word) {
    ...
    if (singularLookup != null) {
      if (singularLookup instanceof ArrayList) return (String) (
        ((ArrayList) singularLookup).get(0)
    ...
    if (
      "sxzo".indexOf(lastLetter) >= 0 ||
      (lastLetter == 'h' && (secondLast == 's' || secondLast == 'c'))
    ) return word + "es";
    if (lastLetter == 'y') {
      if ("aeiou".indexOf(secondLast) >= 0) return word + "s"; else return (
        word.substring(0, length - 1) + "ies"
      );
    }
    return word + "s";
  } // end pluralOf
  ...
  public static String singularOf(String word) {
    val uppercaseIndices = new HashSet<Integer>();
    for (int i = 0; i < word.length(); i++) if (
      Character.toUpperCase(word.charAt(i)) == word.charAt(i)
    ) uppercaseIndices.add(i);
    word = word.toLowerCase();
    if (isSingular(word)) return applyUppercaseIndices(uppercaseIndices, word);
    Object pluralLookup = irregularPlurals.get(word);
    String singular = null;
    if (pluralLookup != null) {
      if (pluralLookup instanceof ArrayList) return applyUppercaseIndices(
        uppercaseIndices,
        (String) (((ArrayList) pluralLookup).get(0))
      ); else return applyUppercaseIndices(
        uppercaseIndices,
        (String) pluralLookup
      );
    }
    int length = word.length();
    if (length <= 1) return applyUppercaseIndices(uppercaseIndices, word);
    char lastLetter = word.charAt(length - 1);
    if (lastLetter != 's') return applyUppercaseIndices(uppercaseIndices, word); // no final -s
    char secondLast = word.charAt(length - 2);
    if (secondLast == '\'') return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 2)
    );
    // remove -'s
    if (word.equalsIgnoreCase("gas")) return applyUppercaseIndices(
      uppercaseIndices,
      word
    );
    if (secondLast != 'e' || length <= 3) return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 1)
    ); // remove final -s
    // Word ends in -es and has length >= 4:
    char thirdLast = word.charAt(length - 3);
    if (thirdLast == 'i') return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 3) + "y"
    ); // -ies => -y
    if (thirdLast == 'x') return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 2)
    ); // -xes => -x
    if (length <= 4) return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 1)
    ); // e.g. uses => use
    char fourthLast = word.charAt(length - 4);
    if (
      thirdLast == 'h' && (fourthLast == 'c' || fourthLast == 's')
    ) return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 2)
    ); // -ches or -shes => -ch or -sh
    if (thirdLast == 's' && fourthLast == 's') return applyUppercaseIndices(
      uppercaseIndices,
      word.substring(0, length - 2)
    ); // -sses => -ss
    return applyUppercaseIndices(
  ...
  private static final Map<String, Object> irregularSingulars = new HashMap<>(
    100
  );
  private static final Map<String, Object> irregularPlurals = new HashMap<>(
    100
  );

  private static String applyUppercaseIndices(
    Set<Integer> indices,
    String word
  ) {
    val builder = new StringBuilder();
    for (int i = 0; i < word.length(); i++) {
      if (indices.contains(i)) builder.append(
