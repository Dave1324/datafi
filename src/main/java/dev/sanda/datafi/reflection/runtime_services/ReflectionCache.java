...
public class ReflectionCache {
  ...
  private void init() {
    ...
    for (Class<?> currentType : dataModelEntityTypes) {
      if (isPersistableEntity(currentType)) entitiesCache.put(
  ...
  private boolean isPersistableEntity(Class<?> currentType) {
    return (
      currentType.isAnnotationPresent(Table.class) ||
      currentType.isAnnotationPresent(Entity.class)
    );
  }
  ...
  private Map<Class<?>, Class<?>> getAnnotatedEntityTypeApiSpecs() {
    return reflections
      .getTypesAnnotatedWith(EntityApiSpec.class)
      .stream()
      .filter(
        clazz ->
          hasOneOfAnnotations(clazz.getSuperclass(), Entity.class, Table.class)
      )
  ...
  private Collection<Method> getPublicMethodsOf(@NonNull Class<?> startClass) {
    List<Method> currentClassMethods = Lists.newArrayList(
      startClass.getMethods()
    );
    Class<?> parentClass = startClass.getSuperclass();
    if (parentClass != null) {
      List<Method> parentClassFields = (List<Method>) getPublicMethodsOf(
        parentClass
      );
  ...
  public static Collection<Field> getClassFields(@NonNull Class<?> startClass) {
    List<Field> currentClassFields = Lists.newArrayList(
      startClass.getDeclaredFields()
    );
    Class<?> parentClass = startClass.getSuperclass();
    if (parentClass != null) {
      List<Field> parentClassFields = (List<Field>) getClassFields(parentClass);
